...
public class GuiComponent implements IGuiComponent {
	...
	public Vector2f getAbsoluteSize() {
		...
		float xUnit = switch (this.sizeConstraint) {
			case Gui.SizeConstraint.XY -> this.size.scaleX * adorneeSize.x;
			case Gui.SizeConstraint.XX -> this.size.scaleX * adorneeSize.x;
			case Gui.SizeConstraint.YY -> this.size.scaleX * adorneeSize.y;
		};

		float yUnit = switch (this.sizeConstraint) {
			case Gui.SizeConstraint.XY -> this.size.scaleY * adorneeSize.y;
			case Gui.SizeConstraint.XX -> this.size.scaleY * adorneeSize.x;
			case Gui.SizeConstraint.YY -> this.size.scaleY * adorneeSize.y;
		};

		float x = xUnit + this.size.offsetX;
		float y = yUnit + this.size.offsetY;
